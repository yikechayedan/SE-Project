...
1. **文件格式验证**: 只有JSON文件会进行格式验证，CSV和ZIP文件会跳过验证
2. **性能优化**: 系统只检查前5个数据项的格式，以提高验证性能
3. **错误信息**: 验证失败时会明确指出哪个数据项缺少哪个必需字段
4. **向后兼容**: 现有数据集的`evaluation_type`字段会默认设置为"subjective"
5. **权限控制**: 只有数据集创建者和管理员可以修改测评类型